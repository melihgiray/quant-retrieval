...
def test_sampling_is_seeded_and_independent_of_input_order():
    queries = pd.DataFrame({"question_id": range(20), "split": ["val"] * 19 + ["test"]})
    selected = sample_queries(queries, 7, 17)
    pd.testing.assert_frame_equal(selected, sample_queries(queries.iloc[::-1], 7, 17))
    assert len(selected) == 7
    assert 19 not in selected.question_id.tolist()
    assert selected.question_id.tolist() != sample_queries(queries, 7, 18).question_id.tolist()


def test_sampling_caps_count_and_rejects_empty_or_held_out_split():
    queries = pd.DataFrame({"question_id": [1], "split": ["val"]})
...
def test_sampling_rejects_duplicate_identity():
    queries = pd.DataFrame({"question_id": [1, 1], "split": ["val", "val"]})
